...
+startRescueMission(D,C,NC) : location(hospital,X,Y) & 
    ...
    <- .count(location(victim,_,_),Vcount);		// Determine the victims
       ...
       .print("Start the Resuce mission for ",C," critical and ",NC,
    		...
    		Vcount, " victimes and ", Ocount," known obstacles. Connect Brick...");
			sendData.
...
+colour(X,Y,green) [source(percept)] 
	<- noVictim.
	
+colour(X,Y,yellow) [source(percept)] 
	<- noVictim.
	
+colour(X,Y,black) [source(percept)] 
	<- noVictim.
